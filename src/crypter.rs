...
use crate::streaming::{StreamBufReader, StreamBufWriter};
type Seq = u64;

fn park_sender<T>(sender: Sender<T>) {
    while !sender.is_empty() {}
}

fn spawn_encrypt<T>(
    ctx: &Ctx,
    receiver: Receiver<(Seq, Vec<u8>)>,
    sender: Sender<T>,
) -> JoinHandle<()>
where
    T: Send + 'static,
    ...
    T: Deserialize,
{
    ...
    spawn(move || {
        ...
        receiver.iter().par_bridge().for_each(|(seq, buf)| {
            let frame = T::encrypt(ctx, buf, seq);

            match sender.send(frame) {
                Ok(()) => {}
                Err(e) => {
                    panic!("error: sender error during encrypt: {:?}", e);
                }
            }
        });
    })
    ...
    T: SetBuf,
{
    let ctx = ctx.clone();
    spawn(move || {
        receiver.iter().par_bridge().for_each(|frame| {
            let f = frame.decrypt(&ctx);
            match sender.send(f) {
                Ok(()) => {}
                Err(e) => {
                    panic!("error: sender error during decrypt: {:?}", e);
                }
            }
        });
    })
            ...
            loop {
                match frames.remove(&seq) {
                    Some(f) => match sender.send(f) {
                        ...
                        Err(e) => {
                            panic!(
                                "order_by_seq send error with frames len of {}, {:?}",
                                frames.len(),
                                e
                            )
                        }
    ...
    */
    ...
    fn test_encrypt_decrypt() {
        ...
        for folder in folders {
            ...
            {
                // read header
                let mut r = BufReader::new(OpenOptions::new().read(true).open(&encrypted).unwrap());
    ...
    fn test_msgs_encrypt_decrypt() {
        ...
        for (msgs, data) in itertools::zip_eq(data_msgs, datas) {
            debug!(
                "encrypt decrypt: processing data of len {} bytes",
                data.len()
            );
            debug!(
                "encrypt decrypt: splitting data of len {} bytes into {} separate channel message",
                data.len(),
                msgs.len()
            );
            ...
            // data == Vec<_>
            {
                ...
                for (i, (pd, d)) in
                    itertools::enumerate(itertools::zip_eq(decrypted.to_vec(), data))
                {
                    if pd != d {
                        debug!("at index {}", i);
                    }
